...
static MPP_RET parse_pps_calingLists(BitReadCtx_t *p_bitctx, H264_SPS_t *sps, H264_PPS_t *pps)
{
    ...
    for (i = 0; i < 6; ++i) {
        READ_ONEBIT(p_bitctx, &pps->pic_scaling_list_present_flag[i], "pic_scaling_list_present_flag");
    ...
    if (pps->transform_8x8_mode_flag) {
        for (i = 0; i < ((sps->chroma_format_idc != 3) ? 2 : 6); ++i) {
            READ_ONEBIT(p_bitctx, &pps->pic_scaling_list_present_flag[i + 6], "pic_scaling_list_present_flag");
...
static MPP_RET parser_pps(BitReadCtx_t *p_bitctx, H264_SPS_t *cur_sps, H264_PPS_t *cur_pps)
{
    ...
    READ_UE(p_bitctx, &cur_pps->num_slice_groups_minus1, "num_slice_groups_minus1");
    VAL_CHECK(ret, cur_pps->num_slice_groups_minus1 <= 1);
    READ_UE(p_bitctx, &cur_pps->num_ref_idx_l0_default_active_minus1, "num_ref_idx_l0_default_active_minus1");
    VAL_CHECK(ret, cur_pps->num_ref_idx_l0_default_active_minus1 < 32);
    READ_UE(p_bitctx, &cur_pps->num_ref_idx_l1_default_active_minus1, "num_ref_idx_l1_default_active_minus1");
    VAL_CHECK(ret, cur_pps->num_ref_idx_l1_default_active_minus1 < 32);
    READ_ONEBIT(p_bitctx, &cur_pps->weighted_pred_flag, "weighted_pred_flag");
    READ_BITS(p_bitctx, 2, &cur_pps->weighted_bipred_idc, "weighted_bipred_idc");
    VAL_CHECK(ret, cur_pps->weighted_bipred_idc < 3);
    READ_SE(p_bitctx, &cur_pps->pic_init_qp_minus26, "pic_init_qp_minus26");
    CHECK_RANGE(p_bitctx, cur_pps->pic_init_qp_minus26, -26, 25);
    READ_SE(p_bitctx, &cur_pps->pic_init_qs_minus26, "pic_init_qs_minus26");
    CHECK_RANGE(p_bitctx, cur_pps->pic_init_qs_minus26, -26, 25);
    READ_SE(p_bitctx, &cur_pps->chroma_qp_index_offset, "chroma_qp_index_offset");
    CHECK_RANGE(p_bitctx, cur_pps->chroma_qp_index_offset, -12, 12);
    cur_pps->second_chroma_qp_index_offset = cur_pps->chroma_qp_index_offset;
    READ_ONEBIT(p_bitctx, &cur_pps->deblocking_filter_control_present_flag, "deblocking_filter_control_present_flag");
    READ_ONEBIT(p_bitctx, &cur_pps->constrained_intra_pred_flag, "constrained_intra_pred_flag");
    READ_ONEBIT(p_bitctx, &cur_pps->redundant_pic_cnt_present_flag, "redundant_pic_cnt_present_flag");
