...
#define H264E_CONTROL_LOG_MODE              0x00000111
...
#define H264E_CONTROL_LOG_DEBUG             0x00001000
...
#define h264e_control_debug_enter() \
    do {\
        if (H264E_CONTROL_LOG_MODE & H264E_CONTROL_LOG_DEBUG)\
...
#define h264e_control_err(fmt, ...)\
    do {\
        if (H264E_CONTROL_LOG_MODE & H264E_CONTROL_LOG_ERROR)\
            { mpp_log(fmt, ## __VA_ARGS__); }\
    } while (0)

#define h264e_control_warning(fmt, ...)\
    do {\
        if (H264E_CONTROL_LOG_MODE & H264E_CONTROL_LOG_WARNNING)\
            { mpp_log(fmt, ## __VA_ARGS__); }\
    } while (0)

#define h264e_control_log(fmt, ...)\
    do {\
        if (H264E_CONTROL_LOG_MODE & H264E_CONTROL_LOG_DEBUG)\
            {  mpp_log(fmt, ## __VA_ARGS__); }\
    } while (0)
