...
static MPP_RET hal_vp8d_vdpu2_init(void *hal, MppHalCfg *cfg)
{
    MPP_RET ret = MPP_OK;
...
static MPP_RET hal_vp8d_vdpu2_deinit(void *hal)
{
    MPP_RET ret = MPP_OK;
    VP8DHalContext_t *ctx = (VP8DHalContext_t *)hal;
                                          ...
                                          HalTaskInfo *task)
{
    ...
    for (i = 0; i < (RK_U32)(1 << pic_param->log2_nbr_of_dct_partitions); i++) {
        ...
        if (i == 0) {
            ...
            if (addr)
                mpp_dev_set_reg_offset(ctx->cfg->dev, 64, addr);
        } else if (i <= 5) {
            regs->reg_dct_strm_base[i - 1] = fd;
            if (addr)
                mpp_dev_set_reg_offset(ctx->cfg->dev, 139 + i, addr);
        } else {
            regs->reg_dct_strm1_base[i - 6] = fd;
            if (addr)
                mpp_dev_set_reg_offset(ctx->cfg->dev, 140 + i, addr);
        }
...
static MPP_RET hal_vp8d_vdpu2_gen_regs(void* hal, HalTaskInfo *task)
{
    MPP_RET ret = MPP_OK;
    ...
    if (!pic_param->frame_type) { //key frame
        ...
        mpp_dev_set_reg_offset(cfg->dev, 131, mb_width * mb_height);
    ...
    if (pic_param->ref_frame_sign_bias_golden) {
        mpp_dev_set_reg_offset(cfg->dev, 136, pic_param->ref_frame_sign_bias_golden);
    }
    ...
    if (pic_param->ref_frame_sign_bias_altref)
        mpp_dev_set_reg_offset(cfg->dev, 137, pic_param->ref_frame_sign_bias_altref);
    ...
    if (pic_param->stVP8Segments.segmentation_enabled + (pic_param->stVP8Segments.update_mb_segmentation_map << 1))
        mpp_dev_set_reg_offset(cfg->dev, 149, pic_param->stVP8Segments.segmentation_enabled
...
static MPP_RET hal_vp8d_vdpu2_start(void *hal, HalTaskInfo *task)
{
    MPP_RET ret = MPP_OK;
    VP8DHalContext_t *ctx = (VP8DHalContext_t *)hal;
...
static MPP_RET hal_vp8d_vdpu2_wait(void *hal, HalTaskInfo *task)
{
    MPP_RET ret = MPP_OK;
    VP8DHalContext_t *ctx = (VP8DHalContext_t *)hal;
    MppDev dev = ctx->cfg->dev;
...
const MppHalApi hal_vp8d_vdpu2 = {
    .name     = "vp8d_vdpu2",
    .type     = MPP_CTX_DEC,
    .coding   = MPP_VIDEO_CodingVP8,
    .ctx_size = sizeof(VP8DHalContext_t),
    .flag     = 0,
    .init     = hal_vp8d_vdpu2_init,
    .deinit   = hal_vp8d_vdpu2_deinit,
    .reg_gen  = hal_vp8d_vdpu2_gen_regs,
    .start    = hal_vp8d_vdpu2_start,
    .wait     = hal_vp8d_vdpu2_wait,
    .reset    = NULL,
    .flush    = NULL,
    .control  = NULL,
...
MPP_DEC_HAL_API_REGISTER(hal_vp8d_vdpu2)
