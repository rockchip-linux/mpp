...
jpegd_set_stream_offset(JpegdHalCtx *ctx, JpegdSyntax *syntax)
{
    ...
    if (offset) {
        mpp_dev_set_reg_offset(ctx->cfg->dev, 64, offset);
    }
...
static MPP_RET jpegd_setup_pp(JpegdHalCtx *ctx, JpegdSyntax *syntax)
{
    ...
    if (ctx->pp_info.pp_enable) {
        ...
        if (uv_offset)
            mpp_dev_set_reg_offset(ctx->cfg->dev, 22, uv_offset);
        ...
        if (uv_offset)
            mpp_dev_set_reg_offset(ctx->cfg->dev, 131, uv_offset);
...
static MPP_RET hal_jpegd_vdpu2_init(void *hal, MppHalCfg *cfg)
{
    MPP_RET ret = MPP_OK;
    ...
    ret = mpp_buffer_get(JpegHalCtx->cfg->buf_group, &JpegHalCtx->pTableBase,
...
static MPP_RET hal_jpegd_vdpu2_deinit(void *hal)
{
    MPP_RET ret = MPP_OK;
    JpegdHalCtx *JpegHalCtx = (JpegdHalCtx *)hal;
...
static MPP_RET hal_jpegd_vdpu2_gen_regs(void *hal,  HalTaskInfo *syn)
{
    jpegd_dbg_func("enter\n");
    ...
    if (syn->dec.valid) {
        ...
        mpp_buf_slot_get_prop(JpegHalCtx->cfg->packet_slots, syn->dec.input,
        ...
        mpp_buf_slot_get_prop(JpegHalCtx->cfg->frame_slots, syn->dec.output,
...
static MPP_RET hal_jpegd_vdpu2_start(void *hal, HalTaskInfo *task)
{
    MPP_RET ret = MPP_OK;
    JpegdHalCtx *JpegHalCtx = (JpegdHalCtx *)hal;
...
static MPP_RET hal_jpegd_vdpu2_wait(void *hal, HalTaskInfo *task)
{
    MPP_RET ret = MPP_OK;
    JpegdHalCtx *JpegHalCtx = (JpegdHalCtx *)hal;
    ...
    ret = mpp_dev_ioctl(JpegHalCtx->cfg->dev, MPP_DEV_CMD_POLL, NULL);
    if (ret) {
        task->dec.flags.parse_err = 1;
...
__SKIP_HARD:
    if (JpegHalCtx->cfg->dec_cb) {
        ...
        mpp_callback(JpegHalCtx->cfg->dec_cb, &param);
    }
    ...
    if (jpegd_debug & JPEGD_DBG_IO) {
        ...
        MppBuffer outputBuf = NULL;
        void *base = NULL;
        mpp_buf_slot_get_prop(JpegHalCtx->cfg->frame_slots, task->dec.output,
                              SLOT_BUFFER, &outputBuf);
...
static MPP_RET hal_jpegd_vdpu2_control(void *hal, MpiCmd cmd_type,
                                       void *param)
{
    jpegd_dbg_func("enter\n");
    MPP_RET ret = MPP_OK;
...
const MppHalApi hal_jpegd_vdpu2 = {
    .name     = "jpegd_vdpu2",
    .type     = MPP_CTX_DEC,
    .coding   = MPP_VIDEO_CodingMJPEG,
    .ctx_size = sizeof(JpegdHalCtx),
    .flag     = 0,
    .init     = hal_jpegd_vdpu2_init,
    .deinit   = hal_jpegd_vdpu2_deinit,
    .reg_gen  = hal_jpegd_vdpu2_gen_regs,
    .start    = hal_jpegd_vdpu2_start,
    .wait     = hal_jpegd_vdpu2_wait,
    .reset    = NULL,
    .flush    = NULL,
    .control  = hal_jpegd_vdpu2_control,
...
MPP_DEC_HAL_API_REGISTER(hal_jpegd_vdpu2)
