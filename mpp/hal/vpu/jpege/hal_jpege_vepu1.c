...
MPP_RET hal_jpege_vepu1_init(void *hal, MppHalCfg *cfg)
{
    ...
    ctx->ioctl_info.regs = mpp_malloc(RK_U32, VEPU_JPEGE_VEPU1_NUM_REGS);
...
#ifdef RKPLATFORM
    if (ctx->vpu_fd >= 0) {
        ret = mpp_device_wait_reg(ctx->vpu_fd, regs, sizeof(jpege_vepu1_reg_set) / sizeof(RK_U32));
    }
#endif
    val = regs[1];
