...
static MPP_RET hal_h265d_vdpu34x_gen_regs(void *hal,  HalTaskInfo *syn)
{
    ...
    hw_regs->common.reg012.wr_ddr_align_en      = (dxva_cxt->pp.tiles_enabled_flag != 0)
                                                  ? 0 : 1;
    ...
    vdpu34x_setup_rcb(&hw_regs->common_addr, cfg->dev, (reg_ctx->fast_mode != 0) ?
...
static MPP_RET hal_h265d_vdpu34x_control(void *hal, MpiCmd cmd_type, void *param)
{
    ...
    case MPP_DEC_SET_FRAME_INFO: {
        ...
        if (MPP_FRAME_FMT_IS_FBC(fmt)) {
            vdpu34x_afbc_align_calc(p_hal->cfg->frame_slots, frame, 16);
        }
        break;
    }
