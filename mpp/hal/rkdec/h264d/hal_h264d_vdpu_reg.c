...
static MPP_RET get_info_input(H264dHalCtx_t *p_hal, H264dVdpuPriv_t *priv)
{
    MPP_RET ret = MPP_ERR_UNKNOW;
...
static MPP_RET fill_picture_entry(DXVA_PicEntry_H264 *pic, RK_U32 index, RK_U32 flag)
{
...
static MPP_RET refill_info_input(H264dHalCtx_t *p_hal, H264dVdpuPriv_t *priv)
{
    MPP_RET ret = MPP_ERR_UNKNOW;
    DXVA_PicParams_H264_MVC  *pp = p_hal->pp;
    {
...
static MPP_RET adjust_input(H264dHalCtx_t *p_hal, H264dVdpuPriv_t *priv)
{
    MPP_RET ret = MPP_ERR_UNKNOW;

    priv->layed_id = p_hal->pp->curr_layer_id;
    get_info_input(p_hal, priv);
...
MPP_RET vdpu_h264d_gen_regs(void *hal, HalTaskInfo *task)
{
    ...
    FUN_CHECK(ret = adjust_input(p_hal, (H264dVdpuPriv_t *)p_hal->priv));
