...
#define MODULE_TAG "hal_h264d_vdpu384b"
...
#define DEC_VDPU384B_REGISTERS               276

#define VDPU384B_SPSPPS_SIZE                 (MPP_ALIGN(2266 + 64, 128) / 8) /* byte, 2266 bit + Reserve 64 */
#define VDPU384B_SCALING_LIST_SIZE           (6*16+2*64 + 128)   /* bytes */
#define VDPU384B_ERROR_INFO_SIZE             (256*144*4)         /* bytes */

#define VDPU384B_ERROR_INFO_ALIGNED_SIZE     (0)
#define VDPU384B_SPSPPS_ALIGNED_SIZE         (MPP_ALIGN(VDPU384B_SPSPPS_SIZE, SZ_4K))
#define VDPU384B_SCALING_LIST_ALIGNED_SIZE   (MPP_ALIGN(VDPU384B_SCALING_LIST_SIZE, SZ_4K))
#define VDPU384B_STREAM_INFO_SET_SIZE        (VDPU384B_SPSPPS_ALIGNED_SIZE + \
                                             VDPU384B_SCALING_LIST_ALIGNED_SIZE)

#define VDPU384B_ERROR_INFO_OFFSET           (0)
#define VDPU384B_STREAM_INFO_OFFSET_BASE     (VDPU384B_ERROR_INFO_OFFSET + VDPU384B_ERROR_INFO_ALIGNED_SIZE)
#define VDPU384B_SPSPPS_OFFSET(pos)          (VDPU384B_STREAM_INFO_OFFSET_BASE + (VDPU384B_STREAM_INFO_SET_SIZE * pos))
#define VDPU384B_SCALING_LIST_OFFSET(pos)    (VDPU384B_SPSPPS_OFFSET(pos) + VDPU384B_SPSPPS_ALIGNED_SIZE)
#define VDPU384B_INFO_BUFFER_SIZE(cnt)       (VDPU384B_STREAM_INFO_OFFSET_BASE + (VDPU384B_STREAM_INFO_SET_SIZE * cnt))

static MPP_RET set_registers(H264dHalCtx_t *p_hal, Vdpu38xRegSet *regs, HalTaskInfo *task)
{
...

    {
        ...
        mpp_buf_slot_get_prop(frm_slots, pp->CurrPic.Index7Bits, SLOT_BUFFER, &mbuffer);
        mpp_buf_slot_get_prop(frm_slots, pp->CurrPic.Index7Bits,
        ...
        case MPP_FRAME_THUMBNAIL_ONLY:
            ...
            regs->comm_addrs.reg192_payload_st_cur_base = fd;
            regs->comm_addrs.reg169_error_ref_base = fd;
            vdpu38x_setup_down_scale(mframe, p_hal->cfg->dev, &regs->ctrl_regs, (void*)&regs->comm_paras);
            break;
...
MPP_RET vdpu384b_h264d_init(void *hal, MppHalCfg *cfg)
{
    ...
    //!< malloc buffers
    reg_ctx->spspps = mpp_calloc(RK_U8, VDPU384B_SPSPPS_SIZE);
    reg_ctx->sclst = mpp_calloc(RK_U8, VDPU384B_SCALING_LIST_SIZE);
    FUN_CHECK(ret = mpp_buffer_get(cfg->buf_group, &reg_ctx->bufs,
                                   VDPU384B_INFO_BUFFER_SIZE(max_cnt)));
    ...
    mpp_slots_set_prop(cfg->frame_slots, SLOTS_LEN_ALIGN, mpp_align_wxh2yuv420);
...
static MPP_RET vdpu384b_h264d_rcb_calc(void *context, RK_U32 *total_size)
{
    ...
    cur_bit_size = MPP_ROUNDUP(16, in_tl_row) * (1.2 * bit_depth + 0.5)
    ...
    cur_bit_size = 0;
    cur_uv_para = vdpu38x_filter_row_uv_coef_map[rcb_fmt];
    cur_bit_size = MPP_ROUNDUP(16, on_tl_row) * (1.2 * bit_depth + 0.5)
                   * (( 6 + 3 * cur_uv_para) * (1 + ctx->mbaff_flag)
                      + 2 * cur_uv_para + 1.5);
...
MPP_RET vdpu384b_h264d_gen_regs(void *hal, HalTaskInfo *task)
{
    ...
    //!< copy spspps datas
    memcpy((char *)ctx->bufs_ptr + ctx->spspps_offset, (char *)ctx->spspps, VDPU384B_SPSPPS_SIZE);

    regs->comm_addrs.reg131_gbl_base = ctx->bufs_fd;
    regs->comm_paras.reg67_global_len = VDPU384B_SPSPPS_SIZE / 16; // 128 bit as unit
    mpp_dev_set_reg_offset(cfg->dev, 131, ctx->spspps_offset);

    if (p_hal->pp->scaleing_list_enable_flag) {
        memcpy((char *)ctx->bufs_ptr + ctx->sclst_offset, (void *)ctx->sclst, VDPU384B_SCALING_LIST_SIZE);
...
MPP_RET vdpu384b_h264d_start(void *hal, HalTaskInfo *task)
{
    MPP_RET ret = MPP_ERR_UNKNOW;
    H264dHalCtx_t *p_hal = (H264dHalCtx_t *)hal;
    INP_CHECK(ret, NULL == p_hal);
...
MPP_RET vdpu384b_h264d_wait(void *hal, HalTaskInfo *task)
{
    MPP_RET ret = MPP_ERR_UNKNOW;
    H264dHalCtx_t *p_hal = (H264dHalCtx_t *)hal;
...
const MppHalApi hal_h264d_vdpu384b = {
    .name     = "h264d_vdpu384b",
    .type     = MPP_CTX_DEC,
    .coding   = MPP_VIDEO_CodingAVC,
    .ctx_size = sizeof(H264dHalCtx_t),
    .flag     = 0,
    .init     = vdpu384b_h264d_init,
    .deinit   = vdpu3xx_h264d_deinit,
    .reg_gen  = vdpu384b_h264d_gen_regs,
    .start    = vdpu384b_h264d_start,
    .wait     = vdpu384b_h264d_wait,
...
MPP_DEC_HAL_API_REGISTER(hal_h264d_vdpu384b)
