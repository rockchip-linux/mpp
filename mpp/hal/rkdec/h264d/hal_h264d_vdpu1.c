                                  ...
                                  H264dVdpu1Regs_t *p_regs)
{
    ...
    //!< set poc to buffer
    {
        ...
        if (pp->field_pic_flag || !pp->MbaffFrameFlag) {
            if (pp->field_pic_flag)
                *pocBase++ = pp->CurrFieldOrderCnt[(pp->CurrPic.AssociatedFlag != 0) ? 1 : 0];
            else
    ...
    //!< stream position update
    {
        ...
        mpp_buf_slot_get_prop(p_hal->cfg->packet_slots, p_hal->in_task->input,
                                   ...
                                   H264dVdpu1Regs_t *p_regs)
{
    ...
    for (i = 0, j = 0xff; i < MPP_ARRAY_ELEMS(pp->RefFrameList); i++) {
        ...
        field_flag = (((pp->RefPicFiledFlags >> i) & 0x1) != 0) ? 0x2 : 0;
        ...
        if (val)
            mpp_dev_set_reg_offset(cfg->dev, vdpu1_ref_idx[i], val);
    ...
    /* inter-view reference picture */
    {
        ...
        if (pp->curr_layer_id && priv->ilt_dpb && priv->ilt_dpb->valid /*pp->inter_view_flag*/) {
            ...
            p_regs->SwReg39.refpic_valid_flag |=
                ((pp->field_pic_flag != 0) ? 0x3 : 0x10000);
        }
    }
    ...
    if (pp->field_pic_flag && pp->CurrPic.AssociatedFlag) {
        mpp_dev_set_reg_offset(cfg->dev, 13, ((pp->wFrameWidthInMbsMinus1 + 1) * 16));
    }
    ...
    p_regs->SwReg05.sw_fieldpic_flag_e = (pp->frame_mbs_only_flag == 0) ? 1 : 0;
    ...
    p_regs->SwReg03.sw_pic_topfield_e = (pp->CurrPic.AssociatedFlag == 0) ? 1 : 0; /* bottomFieldFlag */
...
MPP_RET vdpu1_h264d_init(void *hal, MppHalCfg *cfg)
{
    ...
    //!< malloc buffers
    {
        ...
        for (i = 0; i < loop; i++) {
            FUN_CHECK(ret = mpp_buffer_get(cfg->buf_group, &reg_ctx->reg_buf[i].buf,  buf_size));
...
MPP_RET vdpu1_h264d_start(void *hal, HalTaskInfo *task)
{
    ...
    H264dHalCtx_t *p_hal  = (H264dHalCtx_t *)hal;
    H264dVdpuRegCtx_t *reg_ctx = (H264dVdpuRegCtx_t *)p_hal->reg_ctx;
    H264dVdpu1Regs_t *p_regs = (H264dVdpu1Regs_t *)((p_hal->fast_mode != 0) ?
...
MPP_DEC_HAL_API_REGISTER(hal_h264d_vdpu1)
