...
static MPP_RET hal_av1d_alloc_res(void *hal)
{
    ...
    //!< malloc buffers
    BUF_CHECK(ret, mpp_buffer_get(p_hal->cfg->buf_group, &reg_ctx->bufs, VDPU383_INFO_BUF_SIZE(max_cnt)));
...
MPP_RET vdpu383_av1d_gen_regs(void *hal, HalTaskInfo *task)
{
    ...
    /* set reg -> pkt data */
    {
        ...
        memcpy((char *)ctx->bufs_ptr + ctx->offset_uncomps, (void *)ctx->header_data, VDPU383_UNCMPS_HEADER_SIZE);
