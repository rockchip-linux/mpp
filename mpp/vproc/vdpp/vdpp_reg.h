...
#define VDPP_REG_OFF_DMSR               (0x0080)
...
} RegCom;   // 0x0000 ~ 0x006C, 28 regs

typedef struct vdpp_dmsr_reg {
    struct {
...
} RegDmsr;  // 0x0080 ~ 0x00D0, 21 regs

typedef struct vdpp_zme_reg {
    struct {
        struct {
    ...
    } yrgb_hor_coe; // 0x0000 ~ 0x010c, 68 regs
    ...
    } yrgb_ver_coe; // 0x0200 ~ 0x030c, 68 regs
    ...
    } cbcr_hor_coe; // 0x0400 ~ 0x050c, 68 regs
    ...
    } cbcr_ver_coe; // 0x0600 ~ 0x070c, 68 regs
    ...
    } common;       // 0x0800 ~ 0x0844, 18 regs

} RegZme;   // 0x000 ~ 0x0844, 68*4+18=290(/530) regs

typedef struct vdpp_reg {
    RegCom  common; /* offset: 0x1000, end: 0x106C, size: 112 */
